...
- Self-rated teeth condition is appropriate for a pre-examination questionnaire. In NHANES it was completed during the household interview, separately from the dental examination at the mobile examination center, so the rating is not a restatement of the examiner's findings. Respondents may still have been noticing symptoms (pain, visible decay) at the time they answered, so the rating can reflect caries that were already present. It is therefore best read as a contemporaneous screening signal rather than a prediction of future disease. If someone uses this tool well before any planned dental visit, their self-rating may be a weaker signal than it was in training, so real-world screening accuracy could be lower than the reported results.
